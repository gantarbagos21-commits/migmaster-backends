...
export class MigMasterSession extends DurableObject {
  constructor(ctx, env) {
    ...
    this.commandQueueRunning = false; this.voteKickDetectorIndex = 0; this.autoKick = { enabled:false, room:"", thresholdMs:30000, countdownMs:60000, targets:[], targetDelaysMs:[], loopCount:1, socketDelayMs:0, sequentialMode:false, source:null, countdownEndAt:0, timer:null, triggered:false };
  }
  ...
  beginAutoKick(index,det,source="detected"){if(this.autoKick.timer||!this.autoKick.room)return;this.autoKick.countdownEndAt=Date.now()+this.autoKick.countdownMs;this.autoKick.source=source;this.autoKick.triggered=false;this.send({type:source==="detected"?"autoKick.detected":"autoKick.manual.started",index,room:det.room,countdownMs:this.autoKick.countdownMs,thresholdMs:this.autoKick.thresholdMs,targetUsername:det.targetUsername||"",startedBy:det.startedBy||"",eventId:det.eventId||"",message:det.message||""});this.autoKick.timer=setInterval(()=>{const rem=Math.max(0,this.autoKick.countdownEndAt-Date.now());this.send({type:"autoKick.state",status:this.autoKick.triggered?"triggering":"countdown",enabled:this.autoKick.enabled,room:this.autoKick.room,source:this.autoKick.source,thresholdMs:this.autoKick.thresholdMs,countdownMs:this.autoKick.countdownMs,remainingMs:rem});if(!this.autoKick.triggered&&rem<=this.autoKick.thresholdMs){this.autoKick.triggered=true;this.send({type:"autoKick.triggered",room:this.autoKick.room,thresholdMs:this.autoKick.thresholdMs,remainingMs:rem});if(this.autoKick.targets.length&&!this.commandQueueRunning)this.kickQueue(this.autoKick.room,this.autoKick.targets,0,this.autoKick.loopCount,"autoKick",this.autoKick);}
      if(rem===0){clearInterval(this.autoKick.timer);this.autoKick.timer=null;this.autoKick.countdownEndAt=0;this.autoKick.triggered=false;this.autoKick.source=null;this.send({type:"autoKick.state",status:this.autoKick.enabled?"armed":"disabled",enabled:this.autoKick.enabled,room:this.autoKick.room,remainingMs:0});}},100);}
  ...
  stopAutoKick(reason="stop"){const remaining=this.autoKick.countdownEndAt?Math.max(0,this.autoKick.countdownEndAt-Date.now()):Math.max(0,Number(this.autoKick.remainingMs)||60000);if(this.autoKick.timer)clearInterval(this.autoKick.timer);this.autoKick.timer=null;this.autoKick.countdownEndAt=0;this.autoKick.triggered=false;this.autoKick.source=null;if(reason==="reset"||reason==="autoKick.reset"){this.autoKick.remainingMs=60000;this.autoKick.countdownMs=60000;}else{this.autoKick.remainingMs=remaining;}this.send({type:"autoKick.state",status:this.autoKick.enabled?"armed":"disabled",enabled:this.autoKick.enabled,room:this.autoKick.room,reason,remainingMs:this.autoKick.remainingMs});}
  async onDashboardMessage(raw){let m;try{m=JSON.parse(typeof raw==="string"?raw:new TextDecoder().decode(raw));}catch{return;}const i=Number(m.index);
    ...
    if(m.action==="autoKick.configure"){Object.assign(this.autoKick,{enabled:m.enabled!==false,room:String(m.room||"").trim(),thresholdMs:clampMs(m.thresholdMs),targets:Array.isArray(m.targets)?[...new Set(m.targets.map(x=>String(x||"").trim()).filter(Boolean))].slice(0,10):[],targetDelaysMs:Array.isArray(m.targetDelaysMs)?m.targetDelaysMs.slice(0,10).map(x=>clampMs(x)):[],loopCount:Math.max(1,Math.min(100,Number(m.loopCount)||1)),socketDelayMs:clampMs(m.socketDelayMs),sequentialMode:m.sequentialMode===true});if(!this.autoKick.enabled)this.stopAutoKick("disabled");else this.send({type:"autoKick.state",status:this.autoKick.countdownEndAt?"countdown":"armed",enabled:true,room:this.autoKick.room,remainingMs:Math.max(0,this.autoKick.countdownEndAt-Date.now())});return;}
