...
export class MigMasterSession extends DurableObject {
  constructor(ctx, env) {
    ...
    this.commandQueueRunning = false; this.autoKick = { enabled:false, room:"", thresholdMs:30000, countdownMs:60000, targets:[], targetDelaysMs:[], loopCount:1, socketDelayMs:0, sequentialMode:false, source:null, countdownEndAt:0, timer:null, triggered:false };
  }
  ...
  onUpstream(i,ws,raw) {
    ...
    const det=detectVoteKick(data); if(det)this.startAutoKick(i,det);
  }
  ...
  stopAutoKick(reason="reset"){if(this.autoKick.timer)clearInterval(this.autoKick.timer);this.autoKick.timer=null;this.autoKick.countdownEndAt=0;this.autoKick.triggered=false;this.autoKick.source=null;this.send({type:"autoKick.state",status:this.autoKick.enabled?"armed":"disabled",enabled:this.autoKick.enabled,room:this.autoKick.room,reason,remainingMs:0});}
  async onDashboardMessage(raw){let m;try{m=JSON.parse(typeof raw==="string"?raw:new TextDecoder().decode(raw));}catch{return;}const i=Number(m.index);
    ...
    if(m.action==="autoKick.stop"||m.action==="autoKick.reset"){this.stopAutoKick(m.action);return;}
